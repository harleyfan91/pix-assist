...
import type { AppTabParamList, NavigationProps } from "./AppNavigator"
...
export function getActiveRouteName(state: NavigationState | PartialState<NavigationState>): string {
  ...
  if (!route.state) return route.name as keyof AppTabParamList

  // Recursive call to deal with nested routers
  return getActiveRouteName(route.state as NavigationState<AppTabParamList>)
}
...
export function useNavigationPersistence(storage: Storage, persistenceKey: string) {
  ...
  const routeNameRef = useRef<keyof AppTabParamList | undefined>(undefined)
  ...
  const onNavigationStateChange = (state: NavigationState | undefined) => {
    ...
    if (state !== undefined) {
      ...
      routeNameRef.current = currentRouteName as keyof AppTabParamList
